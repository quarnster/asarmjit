...
void asCJitArm::SplitIntoBlocks(const asDWORD* bytecode, int bytecodeLen)
{
    ...
    while (i <= bytecodeLen)
    {
        ...
        if (i+size < bytecodeLen && *(unsigned char*)&bytecode[i+size] == asBC_SUSPEND)
        {
            isSuspend = true;
            doBlockBreak = true;
        }
        ...
        if (parseCode)
        {
            ...
            switch (opcode)
            {
                ...
                case asBC_CpyVtoV4:
                {
                    ...
                    currSuspend->WriteToRegister(dst);
                    dst = currSuspend->GetNative(dst);
                ...
                */
                ...
                case asBC_LdGRdR4:
                {
                    ...
                    currSuspend->WriteToRegister(asBC_SWORDARG0(bytecode));
                ...
                case asBC_SetV4:
                {
                    ...
                    int arg = asBC_DWORDARG(bytecode);
                    int r = currSuspend->GetNative(reg);
                    currSuspend->WriteToRegister(reg);
